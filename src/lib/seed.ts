...
export async function seedDebugData(actorId: string) {
  ...
  for (const [entryIndex, [leagueId, userId, stockName, symbol, market, currency, startPrice, currentPrice]] of demoEntries.entries()) {
    const existing = await dbGet<{ id: string }>("SELECT id FROM league_entries WHERE league_id = ? AND user_id = ?", [
      leagueId,
      userId,
    ]);
    const league = await dbGet<{ starts_at: string }>("SELECT starts_at FROM leagues WHERE id = ?", [leagueId]);
    if (!league) continue;
    ...
    for (let index = 0; index < 7; index += 1) {
      ...
      statements.push({
        sql: `INSERT INTO price_snapshots (id, league_entry_id, captured_at, price, provider)
          VALUES (?, ?, ?, ?, 'debug-seed')
          ON CONFLICT(id) DO UPDATE SET
            captured_at = excluded.captured_at,
            price = excluded.price,
            provider = excluded.provider`,
        args: [`${entryId}-${index}`, entryId, snapshotDate.toISOString(), Number(price.toFixed(2))],
      });
    }
  }
  ...
  statements.push({
    ...
    args: [actorId, JSON.stringify({ demoUsers: demoUsers.length, demoEntries: demoEntries.length }), now],
  });
